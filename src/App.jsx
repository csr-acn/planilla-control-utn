...
export default function App() {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!err) {
      setNuevaRes({ numero: '', anio: '2026', area: 'ALU', sesion: '', tematica: '', interesado: '', estado: 'sin-redactar', observaciones: '' });
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="bg-white p-1 rounded-xl shadow-md flex items-center justify-center min-w-[44px] h-12">
              ...
              />
              <span className="hidden text-indigo-950 font-bold text-xl px-2">UTN</span>
            </div>
            <div>
          ...
          <div className="flex items-center gap-4">
             <button onClick={() => setShowConfigEstados(true)} className="p-2 hover:bg-indigo-800 rounded-lg text-indigo-200 transition-colors" title="Protocolo de Estados">
               <Settings size={20} />
             </button>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4 flex flex-col justify-center">
             <div className="bg-white p-6 rounded-[2rem] border border-slate-200 flex items-center justify-between shadow-sm transition-transform hover:scale-[1.02]">
                <div>
                  <span className="text-xs font-bold text-slate-400 block mb-1 uppercase tracking-wider">Trámites {dashboardArea === 'Global' ? 'Totales' : dashboardArea}</span>
             ...
             <div className="bg-emerald-50 p-6 rounded-[2rem] border border-emerald-100 flex items-center justify-between shadow-sm transition-transform hover:scale-[1.02]">
                <div>
                  <span className="text-xs font-bold text-emerald-600/60 block mb-1 uppercase tracking-wider">Finalizados</span>
        ...
        <section className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-6 bg-slate-50/50 flex flex-col lg:flex-row justify-between items-center gap-4 border-b border-slate-100">
             <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                <div className="relative flex-grow max-w-md w-full md:w-80">
                  <Search className="absolute left-4 top-3 text-slate-400" size={16} />
                  <input 
                    type="text" 
                    placeholder="Buscar temática o interesado..." 
                    className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm outline-none w-full shadow-sm focus:ring-2 focus:ring-indigo-100 transition-all" 
                    value={filtro} 
                ...
                <div className="flex gap-2">
                  <select 
                    className="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 outline-none" 
                    value={areaFiltro} 
             ...
             <div className="flex gap-2 w-full lg:w-auto">
                <button 
                  onClick={() => { const s = prompt("Sigla nueva área:"); if(s) supabase.from('config_areas').insert([{ sigla: s.toUpperCase() }]); }}
                ...
                <button 
                  ...
                  className="flex-grow bg-indigo-600 text-white px-6 py-2.5 rounded-xl text-xs font-bold uppercase shadow-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-2"
                >
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-indigo-50/10 group transition-colors">
                      ...
                      <td className="px-8 py-5">
                         ...
                         {r.observaciones && (
                           <div className="mt-1 text-[10px] text-slate-400 italic truncate max-w-[150px]">"{r.observaciones}"</div>
                         )}
                      </td>
                      <td className="px-8 py-5 text-center">
                         <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={async () => { if(confirm("¿Eliminar trámite?")) await supabase.from('resoluciones').delete().eq('id', r.id) }} className="text-slate-300 hover:text-red-500 p-2 border border-transparent hover:border-red-100 rounded-lg transition-all" title="Eliminar"><Trash2 size={14} /></button>
                         </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="bg-white rounded-[2.5rem] p-8 md:p-10 max-w-2xl w-full shadow-2xl relative border border-white/20 overflow-y-auto max-h-[95vh]">
             ...
             <form onSubmit={handleCreateSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Estado Inicial</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   ...
                   <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Interesado / Solicitante</label>
                      <input type="text" placeholder="Apellido y Nombre" required className="w-full p-3.5 bg-slate-50 rounded-2xl border-none outline-none text-sm focus:ring-2 focus:ring-indigo-100 shadow-sm" value={nuevaRes.interesado} onChange={e => setNuevaRes({...nuevaRes, interesado: e.target.value})} />
                   </div>
                </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 md:p-10 max-w-2xl w-full shadow-2xl relative border border-white/20 overflow-y-auto max-h-[95vh]">
            ...
            <form onSubmit={handleUpdateSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Estado Actual</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Interesado</label>
